...
def logp(indice, x, y, diag, sigma, term=None, rho=None, period=None, Q0=None, dQ=None, f=None, verbose=False):
    ...
    m2_inv = np.linalg.solve(m2, np.identity(m2.shape[0]))
    ...
    mean = np.dot(m1, np.dot(m2_inv, np.delete(y, indice)))
    cov = m3 - np.dot(m1.T, np.dot(m2_inv, m4))
    ...
    mean2 = y[indice] - np.dot(m5_inv, y)[indice] / m5_inv[indice, indice]
    cov2 = 1 / m5_inv[indice, indice]
    ...
    if verbose:
        ...
        print('indice:', indice, 'cov:', cov, 'mean:', mean, "mean2:", mean2, "cov2:", cov2)
